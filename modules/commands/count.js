module.exports.config = {
	...
	usages: "count message/admin/member/boy/girl/ndfb/allgroup/alluser", // Cách sử dụng lệnh
...
var userget = await Users.getAll(['userID'])
		if (input==""){out(`Bạn chưa nhập thẻ, hãy sử dụng các thẻ sau: mess/admin/member/boy/girl/ndfb/allgroup/alluser`)}
		if (input=="mess") {out(`Nhóm này có ${threadInfo.messageCount} tin nhắn`)}
		...
		if (input=="boy"){out(`Nhóm này có ${gendernam.length} thành viên là nam`)}
		if (input=="girl"){out(`Nhóm này có ${gendernu.length} thành viên là nữ`)}
		if (input=="ndfb"){out(`Nhóm này có ${nope.length} thành viên Người Dùng Facebook`)}
